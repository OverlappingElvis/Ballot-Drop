...
// The form submit
				...
				geocode.geocode( 
					...
					function(results, status) {
						...
						if (status == google.maps.GeocoderStatus.OK ) {
							...
							alert("Hmmm can't find that address holmes, try being more specific");
						}
					}
				);
			});
	...
	if( $(this).is('form') ) {
			...
			$.getJSON(
				'boxes.php?'+$(this).serialize()+'&browser='+browser,
				function(json) {
					response = json;
				}
			);
	...
	$.fn.loadpoints = function() {
		$.getJSON(
			...
			function(json) {
				data = json;
				if( fire ) goToThere(initialLocation);
			}
		);
		return this;
	}
	...
	$.fn.loadme = function() {
		this.ready( function(e) {
			initialize();
			$().loadpoints();
			$('#search').labelify({labelledClass: "labelHighlight" }).searchify();
